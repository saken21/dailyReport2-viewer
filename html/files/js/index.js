...
jp_saken_js_components_Lightbox.resize = function(event) {
	...
	jp_saken_js_components_Lightbox._jContent.css({ left : Math.floor((winW - w) * .5), top : Math.floor((winH - h) * .5), width : w, height : h});
};
...
jp_saken_js_ui_UI.setTab = function(jParents) {
	...
	while(_g1 < _g) init(jParents.eq(_g1++));
};
...
jp_saken_js_utils_Ajax.getMaxData = function(table,column,onLoaded,where) {
	...
	jp_saken_js_utils_Ajax._connectDB.onData = function(data) {
		...
		onLoaded(reg.match(data)?Std.parseInt(reg.matched(0)):0);
		jp_saken_js_utils_Ajax.unsetBusy();
	};
...
jp_saken_js_utils_Ajax.insertData = function(table,columns,values,onLoaded) {
	...
	jp_saken_js_utils_Ajax.requestConnectDB("INSERT INTO " + table + " (" + columns.join(",") + ") VALUES (" + values.join(",") + ")",true);
};
jp_saken_js_utils_Ajax.updateData = function(table,columns,values,where,onLoaded) {
	...
	jp_saken_js_utils_Ajax.requestConnectDB("UPDATE " + table + " SET " + array.join(",") + " WHERE " + where);
};
...
ui_Datalist.set = function(jTarget,mode,clientID) {
	...
	if(clientID != null) {
		if(__map_reserved.clientID != null) {
			_g.setReserved("clientID",clientID);
		} else {
			_g.h["clientID"] = clientID;
		}
	}
	jp_saken_common_utils_API.getJSON("dailyReport2",_g,function(data) {
		var html = "";
		var _g2 = 0;
		var _g1 = data.length;
		while(_g2 < _g1) {
...
utils_Data.getDate = function(datetime) {
	...
	date = date.replace(new RegExp("-","g".split("u").join("")),".");
	return date;
};
...
utils_TimeKeeper.run = function() {
	utils_TimeKeeper._timer = new haxe_Timer(300);
...
view_Information.getData = function(columns,onLoaded) {
	jp_saken_js_utils_Ajax.getData("reports",columns,onLoaded,"updatetime > \"" + view_Information._lastDatetime + "\" AND member_id != " + Manager.myID);
};
...
view_Login.submit = function(event) {
	...
	var columns = ["*"];
	id = id.replace(new RegExp("_","".split("u").join("")),"-");
	jp_saken_js_utils_Ajax.getData("members",columns,view_Login.checkData,"(parmanent_number = \"" + id + "\" OR parmanent_id = \"" + id + "\") AND pass = \"" + pass + "\"");
	if(event != null) {
		event.preventDefault();
	}
};
view_Login.checkData = function(data) {
	...
	var info = data[0];
	js_Cookie.set("DR2LoginHistory2",JSON.stringify([info.parmanent_number,info.pass]));
...
view_ReportViewer.setTotaltime = function(jReport) {
	...
	while(_g1 < _g) {
		var value = jTasks.eq(_g1++).find(".hour").find("input").prop("value");
		if(value.length == 0) {
			value = "0";
		}
...
view_ReportViewer.getChoicedData = function(category,eReg) {
	...
	var getChoiceSearch = function(info4) {
		return eReg.match(view_ReportViewer.getReportString(info4));
	};
	var data = [];
	...
	var _g = view_ReportViewer._fullData.length;
	while(_g1 < _g) {
		var info5 = view_ReportViewer._fullData[_g1++];
		if(getIsMatch(info5)) {
			data.push(info5);
		}
	}
...
view_ReportViewer.addNewEditor = function(myID,date) {
	jp_saken_js_utils_Ajax.getIsEmpty("reports",function(isEmpty) {
		...
		view_ReportViewer._jContents.prepend(view_reportviewer_Html.getReport(0,myID,{ dates : [utils_Data.getDate(date),utils_Data.getDay(date)]},true));
...
view_ReportViewer.addTask = function(jTasks) {
	...
	var scrollTop = 30 * jFocusTask.index();
...
view_ReportViewer.copyTasks = function(jReport) {
	...
	var tmp = "member_id = " + Manager.myID;
...
view_ReportViewer.addStar = function(jReport) {
	...
	jp_saken_js_utils_Ajax.getData("reports",["star_list","is_checked_star"],function(data) {
		...
		jStars.html(view_reportviewer_Html.getStars(array));
		if(isEmpty) {
			array.push(myID);
		...
		jp_saken_js_utils_Ajax.updateData("reports",["star_list","is_checked_star"],[stars,isEmpty?0:checkd],where);
	},where);
};
...
view_SimpleBoard.setEntries = function() {
	jp_saken_js_utils_Ajax.getData("simpleboard",["id","member_id","text","check_list"],function(data) {
		var myID = Manager.myID;
		var html = "";
		data.reverse();
		var _g1 = 0;
...
view_SimpleBoard.addEntry = function(text) {
	var myID = Manager.myID;
	view_SimpleBoard.getDatetime(function(datetime) {
		jp_saken_js_utils_Ajax.insertData("simpleboard",["member_id","text","updatetime","check_list"],[myID,text,datetime,myID],function(lastID) {
			view_SimpleBoard._checkList[lastID] = [myID];
			view_SimpleBoard._jEntries.prepend(view_SimpleBoard.getEntryHTML(lastID,myID,jp_saken_common_utils_Handy.getAdjustedHTML(text),true,1)).find("li").eq(0).hide().slideDown(500);
		});
	});
...
view_SimpleBoard.deleteEntry = function(jTarget) {
	...
	if(!window.confirm("「" + text + "」を削除してもよろしいですか？")) {
		return;
	}
	jTarget.remove();
...
view_StarChecker.onLoop = function() {
	var tmp = "is_checked_star = 0 AND member_id = " + Manager.myID;
...
view_StarChecker.set = function(data) {
	...
	while(_g1 < _g) {
		...
		html += "<li data-id=\"" + id + "\"><a class=\"search\" data-search=\"" + search + "\">" + ("あなたの" + date + "の日報に" + member1 + member2 + others + "がスターを付けてます") + "</a></li>";
...
view_StarChecker.onClickWindow = function(event) {
	if($(event.target).parents("#starchecker").length > 0) {
		return;
	}
	view_StarChecker.close();
};
...
view_reportviewer_Html.getTasks = function(tasks,isCopy) {
	...
	var _g = tasks.length;
	while(_g1 < _g) {
		var taskID = tasks[_g1++];
		if(taskID > 0) {
			var taskInfo = db_Tasks.db[taskID];
			var workID = taskInfo.work_id;
			var workInfo = db_Works.db[workID];
			var clientID = workInfo.client_id;
			html += view_reportviewer_Html.getTask([isCopy?"0":taskID,clientID,db_Clients.db[clientID].name,workID,workInfo.name,taskInfo.hour]);
		} else {
...
view_reportviewer_Html.getDatalist = function(data) {
	...
	while(_g1 < _g) {
		...
		if(info != null) {
			...
			html += "<option value=\"" + Std.string(info.name) + "\" data-id=\"" + Std.string(info.id) + "\"" + (clientID == null?"":" data-clientid=\"" + clientID + "\"") + "></option>";
		}
	}
	return html;
};
view_reportviewer_Html.getImages = function(images) {
	...
	var _g1 = 0;
	var _g = images.length;
	while(_g1 < _g) html += view_reportviewer_Html.getImage(images[_g1++]);
	return html;
};
...
view_reportviewer_ReportEditor.getIsFull = function() {
	if(view_reportviewer_ReportEditor._jTaskList.length > 1) {
		...
		while(_g1 < _g) {
			var jTask = view_reportviewer_ReportEditor._jTaskList.eq(_g1++);
			var isEmpty = true;
			var jInputs = jTask.find("input");
			var _g3 = 0;
			var _g2 = jInputs.length;
			while(_g3 < _g2) if(jInputs.eq(_g3++).prop("value").length > 0) {
				isEmpty = false;
			}
			if(isEmpty) {
				jTask.remove();
	...
	while(_g11 < _g4) if(jInputs1.eq(_g11++).prop("value").length == 0) {
		isFull = false;
	}
	return isFull;
};
view_reportviewer_ReportEditor.updateTasks = function() {
	...
	var _g1 = 0;
	var _g = counter;
	while(_g1 < _g) new view_reportviewer_Task(view_reportviewer_ReportEditor._jTaskList.eq(_g1++).unbind().on("updated",null,onUpdated));
};
...
view_reportviewer_ReportEditor.updateData = function() {
	...
	var _g1 = 0;
	var _g = view_reportviewer_ReportEditor._jTaskList.length;
	while(_g1 < _g) taskIDs.push(view_reportviewer_ReportEditor._jTaskList.eq(_g1++).data("id"));
	var _g11 = 0;
	var _g2 = jImages.length;
	while(_g11 < _g2) images.push(jImages.eq(_g11++).data("filename"));
	...
	jp_saken_js_utils_Ajax.getDatetime(function(datetime) {
		...
		if(id > 0) {
			jp_saken_js_utils_Ajax.updateData("reports",colums,values,"id = " + id,view_reportviewer_ReportEditor.onUpdated);
		} else {
			colums = colums.concat(["date"]);
			values.push(datetime);
			jp_saken_js_utils_Ajax.insertData("reports",colums,values,view_reportviewer_ReportEditor.onInsertedData);
		}
	});
};
...
view_reportviewer_ReportEditor.onUpdated = function() {
	view_reportviewer_ReportEditor._isUpdating = false;
	view_reportviewer_ReportEditor._jParent.stop().removeClass("edit-mode" + " " + "updating").animate({ width : 298},300);
...
view_reportviewer_Scrollbar.onScroll = function(event) {
	if(Manager.jWindow().scrollTop() - (view_reportviewer_Scrollbar._jTarget.position().top + view_reportviewer_Scrollbar._jTarget.height()) > 100 - view_reportviewer_Scrollbar._jTarget.find(".report").first().height() * 2) {
		view_ReportViewer.loadMore();
	}
};
...
view_reportviewer_SortNavi.onMousedown = function(event) {
	...
	jTarget.parents("ul").find("button").removeClass("active");
...
view_reportviewer_Task.prototype = {
	...
	,update: function() {
		...
		jp_saken_js_utils_Ajax.getDatetime(function(datetime) {
			...
			if(_gthis._id > 0) {
				jp_saken_js_utils_Ajax.updateData("tasks",columns,values,"id = " + _gthis._id,function() {
					_gthis._jParent.trigger("updated");
				});
			} else {
				jp_saken_js_utils_Ajax.insertData("tasks",columns,values,function(lastID) {
